  ...
  F && f )
{
  ...
  {
    Spherical< T > const r2 { r2V[ 0 ], r2V[ 1 ], 0 };

    T const answer = r2BasisValues( idx, b2 ) * r2BasisValues( idx, b4 ) * f( r1, r2 );
    ++idx;
    return answer;
  } );
}
      ...
      for( IndexType b2 = 0; b2 < nBasis; ++b2 )
      {
        ...
        for( IndexType b4 = 0; b4 < nBasis; ++b4 )
        {
          ...
          for( IndexType b1 = 0; b1 < nBasis; ++b1 )
          {
            ...
            {
              std::complex< T > const addition = innerIntegral * r1Weight * std::conj( r1BasisValues( idx, b1 ) ) * r1BasisValues( idx, b3 );
